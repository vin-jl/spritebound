...
public class Battle {
    ...
    public Battle(BattleCharacter[] characters, BattleEnemy enemy, Item equippedItem){
        ...
        curCharacter = 1;
        if(equippedItem == null){status = BattleStage.SELECTION;}
        else{status = BattleStage.BATTLE;}
    ...
    private void drawMenuTriangle(Graphics g, int x){
        g.setColor(Color.WHITE);
        //draws triangle based on menu choice
        switch(menuChoice){
            case ATTACK:
    ...
    private void battlePhase(Graphics g){
        ...
        if(equippedItem == null) {
            ...
            if (lastDialogue) {
                if (!nextDialogue) {
                    ...
                    if (aliveCharacters.isEmpty()) {
                        status = BattleStage.LOSE;
                    }
                    //else if win (enemy dead)
                    else if (!enemy.isAlive()) {
                        status = BattleStage.WIN;
                    } else {
                        resetTurn();
                    }
                }
            ...
            //if has not used equippeditem yet
            actionDialogue = equippedItem.getDescription();
            battleDialogue(g);
            System.out.println(actionDialogue);
            if(nextDialogue){
                //give item buffs
                if(equippedItem == CONST.ITEM_PENGUIN){
